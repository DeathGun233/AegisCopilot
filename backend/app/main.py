...
def _friendly_index_state(index_state: DocumentIndexState) -> str:
    return {
        ...
        DocumentIndexState.indexing: "???",
        DocumentIndexState.indexed: "???",
...
def _friendly_task_kind(kind: DocumentTaskKind) -> str:
    return {
        DocumentTaskKind.upload: "????",
        DocumentTaskKind.reindex: "????",
    }[kind]
...
def _build_document_summary(document: Document, chunk_count: int, task: DocumentTask | None = None) -> DocumentSummary:
    ...
    if chunk_count <= 0 and document.index_state == DocumentIndexState.indexed:
        ...
        embedding_label = f"??????? {embedded_chunk_count} ???"
    else:
        embedding_label = f"??? {missing_embedding_chunks} / {chunk_count} ???"
...
def index_document(
    ...
    if not document_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="?? document_id")
    container = get_container()
    try:
